...
![[Pasted image 20230130200323.png]]
>插入气泡以暂停流水线知道ret到达写回再取出指令进行译码。

知道`ret`到达写回阶段之前都会不断读取错误的指令，但是均会在译码阶段替换成气泡，然后经过剩下的流水阶段。

#### 预测错误的分支
 **预测错误的分支**：在分支逻辑发现不应该选择分支之前，分支目标处的几条指令已经进人流水线了。必须取消这些指令，并从跳转指令后面的那条指令开始取指。
>**当跳转指令到达执行阶段时就可以检测到预测错误**。然后在下一个时钟周期，控制逻辑就会在译码和执行段插人气泡，取消两条不正确的已取指令。在同一个时钟周期，流水线将正确的指令读取到取指阶段。
![[跳转.gif]]

#### 异常
**异常**：当一条指令导致异常，我们想要禁止后面的指令更新程序员可见的状态，并且在异常指令到达写回阶段时，停止执行。

对于导致异常的指令，在前面所有的指令结束前，后面的指令不能影响程序的状态。我们将采取的措施有：
1. 禁止执行阶段中的指令设置条件码
2. 向内存阶段中插人气泡，以禁止向数据内存中写人
3. 当写回阶段中有异常指令时，暂停写回阶段，因而暂停了流水线。
![[Pasted image 20230130202918.png]]

#### 多种组合错误的解决方法：
例子一：
- 分支预测错误+`ret`
![[Pasted image 20230130204322.png]]
正确执行应该是指令`irmovq`，但是由于分支预测错误将会执行`ret`指令。
- 解决方法：
![[组合冒险.gif]]
`jne`译码阶段会发现错误，PC的选择逻辑单元会选择跳转指令的下一条指令。

例子一：
- 加载/使用冒险+`ret`
![[Pasted image 20230130204858.png]]
`mrmovq`指令使用的寄存器是`rsp`，`ret`指令也是使用`rsp`中的内存地址在内存中读取返回指令。
- 解决方法：
![[Pasted image 20230130205332.png]]
>对F进行暂停，禁止取出下一条指令
>对D进行暂停和插入气泡，实际我们只进行暂停就可
>对E插入气泡

### 4.5.9 性能分析
确定往流水线中插人气泡的频率，来衡量这种效率的损失，因为插入气泡会导致未使用的流水线周期。

**指令周期数（CPI）**：**执行一条指令**所需要的**平均时钟周期**数的估计值。
>[!note]
>**CPI与吞吐量**的关系=互为倒数！不过时间单位是时钟周期，而不是微微秒。

![[Pasted image 20230130205820.png]]
其中，$C_i$是指令数，$C_b$是气泡数。$C_i/C_b$这个项表明执行一条指令平均要插人多少个气泡。.
因为只有三种指令类型会导致插人气泡，我们可以将这个**处罚项分解成三个部分**：
![[Pasted image 20230130210007.png]]、
其中，
- $lp$（加载处罚）是当由于加载/使用冒险造成暂停时插人气泡的平均数。
- $mp$（预测错误分支处罚）是当由于预测错误取消指令时插人气泡的平均数
- $rp$（返回处罚）当由于`ret`指令造成暂停时插入气泡的平均数。
每种处罚都是由该种原因引起的插入气泡的总数（$C_b$）的一部分。

CPI例子：
![[Pasted image 20230130212124.png]]
...
## 5.1 优化编译器的能力和局限性
![[Pasted image 20230201203455.png]]
当两个参数指针指向相同位置时，两个函数执行结果不同，因此第二个不能作为第一个的优化方案，尽管它更加高效。

![[Pasted image 20230201203619.png]]
![[Pasted image 20230201203631.png]]
两个函数的执行结果还是不一样，因为涉及到全局变量，改变调用次数会改变程序的行为。对`func1`的调用返回$0+1+2+3=6$.而对`func`的调用会返回$4*0=0$.

大多数编译器不会试图判断一个函数是否没有副作用，如果没有，就可能被优化成像`func2`中的样子。相反，编译器会假设最糟的情况，并保持所有的函数调用不变。

## 5.2 表示程序性能

- **每元素的周期数（CPE）**：一种表示程序性能并指导我们改进代码的方法。

- **千置和**：
![[Pasted image 20230201204354.png]]
![[Pasted image 20230201204401.png]]
![[Pasted image 20230201204332.png]]

- 计算前置和函数：
![[Pasted image 20230201204413.png]]
函数`psuml`每次迭代计算结果向量的一个元素。第二个函数使用**循环展开**技术。
使用最小二乘法拟合，元素与周期的关系发现：
![[Pasted image 20230201204614.png]]
**元素有效周期数(CPE)：** 拟合曲线的系数。

## 5.3 程序示例
- 程序定义：
![[Pasted image 20230201205152.png]]
![[Pasted image 20230201212225.png]]![[Pasted image 20230201212229.png]]

CPE度量值：
![[Pasted image 20230201212324.png]]
未经优化的代码是从C语言代码到机器代码的直接翻译，通常效率明显较低。`-O1`会进行一些基本的优化

## 5.4 消除循环的低效率
- 上述代码中，对于循环不变量中的函数调用，每一次循环都会调用一次，效率较低。
- **解决方法：** 将其使用一个变量进行保存。使得函数只调用一次。
![[Pasted image 20230201212620.png]]

- 优化结果：
![[Pasted image 20230201212633.png]]

优化编译器会试着进行代码移动。但是不能发现是否存在副作用，所以都会假设存在副作用。一些代码的改进则需要程序员手动完成代码的移动。

## 5.5 减少过程调用
![[Pasted image 20230201212954.png]]
![[Pasted image 20230201213030.png]]

- 性能没有明显的提升。可以推断循环中还有其他因素的影响。

## 5.6 消除不必要的内存引用
- 汇编代码
![[Pasted image 20230201213431.png]]

- 存在的问题：
`rbx`保存每次计算的结果在内存中的地址，在程序开始需要从内存中读取，计算之后又要写入内存。

- 我们可以**消除不必要的内存读写**：引入临时变量`acc`，用于保存循环中累计计算的值，只有循环结束后才存放在`dest`中。
![[Pasted image 20230201213649.png]]
![[Pasted image 20230201214242.png]]
>优化之后我们只需要在，中间的累加结果保存在寄存器`xmm`中，因为下一次循环都是用上一次累加结果再加上数组中的一项。内存的操作从两次读和一次写减少到了一次读。
![[Pasted image 20230201214317.png]]
![[Pasted image 20230201214534.png]]

## 5.7 理解现代处理器
- **指令级并行**：同时对多条指令求值。
- 两种下界描述了程序的最大性能。
	- 当一系列操作**必须按照严格顺序**执行时，就会遇到**延迟界限**，因为在下一条指令开始之前，这条指令必须结束。当代码中的数据相关限制了处理器利用指令级并行的能力时，延迟界限能够限制程序性能。
	- 吞吐量界限：刻画了**处理器功能单元的原始计算**能力。这个界限是程序性能的终极限制。

### 5.7.1整体操作
- **超标量处理器**：可以在**每个时钟周期**执行**多个操作**，而且是**乱序**的，就是指令执行的顺序不一定要与它们在机器级程序中的顺序一致。

- 一个乱序处理器设计有两个主要部分
	- **指令控制单元**（ICU）：从内存中读出指令序列，并根据这些指令序列生成一组针对程序数据的基本操作；
	- **执行单元**（EU）：执行ICU生成的操作。

![[Pasted image 20230203204822.png]]
- **取指控制的块**：包括**分支预测**，以完成确定取哪些指令的任务。
	- **分支预测技术**：当遇到分支时，处理器会猜测是否会选择分支，同时还预测分支的目标地址。
	- **投机执行技术**：处理器会开始取出位于它预测的分支会跳到的地方的指令，并对指令译码，甚至在它确定分支预测是否正确之前就开始执行这些操作。如果过后确定分支预测错误，会将状态重新设置到分支点的状态，并开始取出和执行另一个方向上的指令。
- **指令译码逻辑**：接收实际的程序指令，并将它们转换成一组基本操作（有时称为微操作)。每个这样的操作都完成某个简单的计算任务。
>[!note]
>`addq %rax,8(%rdx)`指令会被译码成三个操作。
>1. 从内存中加载一个值到处理器
>2. 将加载进来的值加上寄存器%rax中的值
>3. 将执行结果存回内存。
>这种**译码逻辑对指令进行分解，允许任务在一组专门的硬件单元之间进行分割**。这些单元可以并行地执行多条指令的不同部分。

- **执行单元**：接收来自取指单元的操作。通常，每个时钟周期会接收多个操作。这些操作会被分派到一组功能单元中，它们会执行实际的操作。最终结果不会存放在程序寄存器或数据内存中，直到处理器能确定应该实际执行这些指令。
	- **分支操作**会被送到**EU**确定分支预测**是否正确**
	- 预测错误：**EU**会丢弃分支点之后计算出来的结果，它还会发信号给**分支单元**，说预测是错误的，并**指出正确的分支目的**。之后分支单元开始在正确的目的进行取值。

- **加载和存储单元**：用于读写内存，它们各有一个加法器来完成地址计算。通过数据高速缓存来访问内存。
	- **加载单元处理**从内存读数据到处理器的操作。
	- **存储单元处理**从处理器写数据到内存的操作。
	- **数据高速缓存**：一个高速存储器，存放着最近访问的数据值。

- **退役单元**：**记录**正在进行的处理，并确保它遵守机器级程序的顺序语义。
>指令译码时，关于指令的信息被放置在一个先进先出的队列中。这个信息会一直保持在队列中，直到发生以下两个结果中的一个。
>- 首先，一旦一条指令的操作完成了，而且所有引起这条指令的分支点也都被确认为预测**正确**，那么这条指令就可以**退役**了，所有对程序寄存器的**更新**都可以被实际**执行**了。
>- 另一方面，如果引起该指令的某个分支点预测**错误**，这条指令会被**清空**，**丢弃**所有计算出来的结果。

- **操作结果**：执行单元之间回进行信息交换，加速一条指令到零一条指令结果的传输。
	- **寄存器重命名**机制：控制操作数在执行单元间传送。
> 当一条更新寄存器r的指令译码时，产生标记t，得到一个指向该操作结果的唯一的标识符。条目(r,t)被加入一张表中，该表维护者每个程序寄存器r会与更新该寄存器的操作的标记t之间的关联。当随后以寄存器作为操作数的指令译码时，发送到执行单元的操作会包含t作为操作数源的值。当某个执行单元完成第一个操作时，会生成一个结果(v,t)，指明标记为t的操作产生值v。所有等待作为源的操作都能使用v作为源值，这就是一种形式的数据转发。

微处理器芯片上，一个功能单元能够执行多个操作，一个操作可以在多个功能上执行，功能单元的这种组合**具有同时执行多个同类型操作**的潜力。

### 5.7.2 功能单元的性能
- **延迟**：延迟表明执行实际运算所需要的时钟周期总数。
- **发射**：发射时间表明两次运算之间间隔的最小周期数。
- **容量**：容量表明同时能发射多少个这样的操作。
![[Pasted image 20230203213213.png]]
- **完全流水线化的**：指发射时间都为1的 单元。
- 加法和乘法运算的发射时间都为1，意思是说在每个时钟周期，处理器都可以开始一条新的这样的运算。发射时间为1的功能单元被称为完全流水线化的：每个时钟周期可以开始一个新的运算。
- **最大吞吐量**：定义为发射时间的倒数。一个完全流水线化的功能单元有最大的吞吐量，每个时钟周期一个运算，而发射时间较大的功能单元的最大吞吐量比较小。
